...
class LinearModel(nn.Module):
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        ...
        output = F.relu(self.bn(self.fc_input(output)))
...
class ConvModel(nn.Module):
    ...
    def __init__(self, out_channels: int, hidden_dim: int, input_dim: int, 
        ...
        self.fc_output = nn.Linear(in_features=hidden_dim, out_features=output_dim)
        self.pool_input = nn.MaxPool1d(kernel_size=3)
        self.bn = nn.BatchNorm1d(num_features=hidden_dim)
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        ...
        output = F.relu(self.pool_input(self.conv_input(output)))
        # output = F.relu(self.pool_hidden(self.conv_hidden(output)))
